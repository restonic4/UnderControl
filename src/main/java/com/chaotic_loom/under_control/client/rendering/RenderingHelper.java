...
public class RenderingHelper {
    public static abstract class Geometry {
        ...
        public static void renderBeam3D(PoseStack poseStack, Matrix4f matrix4f, Camera camera, float x, float y, float z, float width, float height) {
            ...
            for (int i = 0; i < vector3fList.size(); i++) {
                ...
                Geometry.renderDynamicGeometry(poseStack, matrix4f, camera, VertexFormat.Mode.TRIANGLE_FAN, vector3fs, UnderControlShaders.SIMPLE_COLOR);
            }
        }
        ...
        public static void renderGeometry(List<Vector3f[]> vector3fList, VertexFormat.Mode vertexFormatMode, ShaderHolder shaderHolder, PoseStack poseStack, Matrix4f matrix4f, Camera camera, float x, float y, float z, float xScale, float yScale, float zScale, float xRotation, float yRotation, float zRotation, int flags) {
            for (int i = 0; i < vector3fList.size(); i++) {
                ...
                Geometry.renderDynamicGeometry(poseStack, matrix4f, camera, vertexFormatMode, vector3fs, shaderHolder, flags);
            }
        }
    ...
    public static abstract class RenderLayers {
        private static Map<String, RenderLayerRenderer<? extends GeometryData>> renderLayers = new HashMap<>();

        public static void registerRenderLayer(RenderLayerRenderer<? extends GeometryData> renderer) {
            RenderSystem.assertOnRenderThread();
            renderLayers.put(renderer.getId(), renderer);
        }

        public static void removeRenderLayer(RenderLayerRenderer<? extends GeometryData> renderer) {
            RenderSystem.assertOnRenderThread();
            renderLayers.remove(renderer.getId());
        }

        public static RenderLayerRenderer<? extends GeometryData> getRenderLayer(String id) {
            return renderLayers.get(id);
        }

        public static void renderLayers(WorldRenderContext context) {
            ...
            for (Map.Entry<String, RenderLayerRenderer<? extends GeometryData>> entry : renderLayers.entrySet()) {
                RenderLayerRenderer<? extends GeometryData> renderLayer = entry.getValue();

                renderLayer.render(poseStack, bufferSource);
            }
        ...
        public static void addVertex(VertexConsumer consumer, Matrix4f matrix, float x, float y, float z, float u, float v, float r, float g, float b, float a, float nx, float ny, float nz) {
